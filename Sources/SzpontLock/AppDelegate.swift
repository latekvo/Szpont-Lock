...
final class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        SecretStore.log("LAUNCHED (panic timeout: \(panic))")
    ...
    private func runFirstLaunchSetup() {
        ...
        alert.informativeText = """
            SzpontLock lives in the menu bar. Arm it and the screen stays on and the \
            mouse keeps working - but the first keystroke locks the machine, photographs \
            whoever is at the keyboard, and swallows all input until you unlock with \
            Touch ID or your unlock sequence.
    ...
    func applicationShouldTerminate(_ sender: NSApplication) -> NSApplication.TerminateReply {
        // Quitting while locked would hand the machine back, so refuse.
        guard controller.state != .locked else { return .terminateCancel }
        return .terminateNow
    }
